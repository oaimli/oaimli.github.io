...
特征统计独立，每个特征具有相同的方差$\sigma^2$，协方差矩阵为对角阵，$|\pmb \Sigma_i|=\sigma^{2d}$，$\pmb \Sigma_i^{-1}=\frac{1}{\sigma^2} \mathbf{I}$
...
数据集上估计离散空间的类条件概率密度$P(e_1e_2\cdots e_K|\omega_i)$，指数级复杂度，需要大量样本
...
- 参数法(见参数估计)：假定概率密度函数形式，$p(\mathbf{x} \mid \omega_i) = p(\mathbf{x} \mid \theta_i)$
